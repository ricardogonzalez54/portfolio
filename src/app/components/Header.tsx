...
export default function Header() {
  ...
  // useEffect solo se ejecuta en el cliente, así que podemos mostrar la UI de forma segura
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        // Actualizar el mapa de secciones visibles
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            currentSections.set(entry.target.id, entry.intersectionRatio);
          } else {
  ...
  const ThemeToggleButton = () => {
    ...
    return (
      <button
        onClick={toggleTheme}
        className="cursor-pointer p-2 rounded-lg bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-500 transition-colors duration-200"
      ...
      <nav className="container mx-auto px-4 py-4 max-w-screen md:max-w-6xl">
        <div className="flex justify-between items-center">
          ...
          <div className="flex space-x-4">
            ...
            <a
              href="https://www.linkedin.com/in/ricardo-gonz%C3%A1lez-fritz-325264283/"
              target="_blank"
              rel="noopener noreferrer"
              className="text-gray-600 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400 transition-colors"
              aria-label="LinkedIn"
            ...
            <a
              href="https://x.com/RicardoGonDev"
              target="_blank"
              rel="noopener noreferrer"
              className="text-gray-600 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400 transition-colors"
              aria-label="Twitter"
